...
func (h *ProxyHandler) authenticateRequest(w http.ResponseWriter, r *http.Request, serverName string) bool {
	...
	if h.oauthEnabled && h.authServer != nil {
		...
		if err == nil && accessToken != nil {
			// OAuth token is valid; we return immediately on success, so the
			// authenticatedViaOAuth flag below is only meaningful on the failure
			// path (where it stays false).
	...
	// We return immediately on success, so authenticatedViaAPIKey only matters
	// in the failure-path checks below (where it stays false).
