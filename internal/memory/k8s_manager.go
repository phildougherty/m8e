...
func (m *K8sManager) Start() error {
	...
	// Check if MCPMemory resource already exists. Bound API calls so a slow
	// API server cannot stall lifecycle operations indefinitely.
...
func (m *K8sManager) Restart() error {
	...
	// Wait for the MCPMemory resource to actually be deleted before recreating
	// it, instead of guessing with a fixed sleep. The 15s parent deadline
	// must outlast the 10s poll timeout so PollUntilContextTimeout can return
	// its own (more informative) timeout error rather than ctx.Deadline.
	...
	_ = wait.PollUntilContextTimeout(ctx, 200*time.Millisecond, 10*time.Second, true, func(ctx context.Context) (bool, error) {
		mem := &crd.MCPMemory{}
		getErr := m.client.Get(ctx, types.NamespacedName{Name: "memory", Namespace: m.namespace}, mem)
...
func (m *K8sManager) createMemoryResource() *crd.MCPMemory {
	...
	if m.config != nil {
		...
		// Add database URL if specified. When empty, the MCPMemory controller
		// builds the URL itself from PostgresEnabled + <name>-postgres, which
		// is the sidecar service it actually creates. Don't synthesize a
		// "matey-postgres" URL here — that host doesn't exist unless the user
		// has separately installed the shared MCPPostgres.
		if mem.DatabaseURL != "" {
