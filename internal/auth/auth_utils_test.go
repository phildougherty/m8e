...
func TestDefaultCodeVerifier_GenerateCodeChallenge(t *testing.T) {
	tests := []struct {
		...
		{
			// OAuth 2.1 / RFC 7636 §7.2 deprecate "plain"; we reject it.
		...
		{
			name:        "empty verifier with plain is rejected",
			verifier:    "",
			method:      "plain",
			expectError: true,
		},
...
func TestDefaultCodeVerifier_VerifyCodeChallenge(t *testing.T) {
	// Compute a real S256 challenge so we have a positive-path case as well
	// as the various negative paths.
	v := &DefaultCodeVerifier{}
	validVerifier := "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk"
	validS256Challenge, err := v.GenerateCodeChallenge(validVerifier, "S256")
	if err != nil {
		t.Fatalf("GenerateCodeChallenge(S256) returned unexpected error: %v", err)
	}
	...
	tests := []struct {
		...
		{
			// OAuth 2.1 / RFC 7636 §7.2: "plain" is deprecated and must
			// not be honoured even when verifier == challenge.
			name:      "plain method always rejected (matching verifier)",
		...
		{
			name:      "plain method always rejected (mismatched)",
		...
		{
			name:      "S256 with non-matching challenge",
			challenge: "definitely-not-the-right-hash",
			verifier:  validVerifier,
			method:    "S256",
			expected:  false,
		},
