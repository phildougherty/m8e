...
func TestJWTTokenGeneration(t *testing.T) {
	...
	t.Run("pkce_code_challenge", func(t *testing.T) {
		...
		// The "plain" PKCE method is deprecated by OAuth 2.1 / RFC 7636 §7.2
		// and must be rejected by both Generate and Verify paths.
		if _, err := verifier.GenerateCodeChallenge(codeVerifier, "plain"); err == nil {
			t.Error("GenerateCodeChallenge(..., \"plain\") should return an error; \"plain\" is deprecated")
		}
		if verifier.VerifyCodeChallenge(codeVerifier, codeVerifier, "plain") {
			t.Error("VerifyCodeChallenge(..., \"plain\") should always return false; \"plain\" is deprecated")
		}
	})
}
...
func TestResourceMetadataHandler_SetterMethods(t *testing.T) {
	// Test ResourceMetadataHandler setter methods for full coverage
	handler := NewResourceMetadataHandler([]string{"https://auth.example.com"}, []string{"read", "write"})

	// Test SetResource
	handler.SetResource("test-resource")
	if handler.GetMetadata().Resource != "test-resource" {
		t.Error("SetResource failed")
	}

	// Test SetJWKSUri
	handler.SetJWKSUri("https://example.com/.well-known/jwks.json")
	if handler.GetMetadata().JWKSUri != "https://example.com/.well-known/jwks.json" {
		t.Error("SetJWKSUri failed")
	}

	// Test SetDocumentation
	handler.SetDocumentation("https://example.com/docs")
	if handler.GetMetadata().ResourceDocumentation != "https://example.com/docs" {
		t.Error("SetDocumentation failed")
	}

	// Test SetPolicyURI
	handler.SetPolicyURI("https://example.com/policy")
	if handler.GetMetadata().ResourcePolicyURI != "https://example.com/policy" {
		t.Error("SetPolicyURI failed")
	}

	// Test SetTosURI
	handler.SetTosURI("https://example.com/tos")
	if handler.GetMetadata().ResourceTosURI != "https://example.com/tos" {
		t.Error("SetTosURI failed")
	}

	// Test AddAuthorizationServer
	handler.AddAuthorizationServer("https://auth2.example.com")
	servers := handler.GetMetadata().AuthorizationServers
	if len(servers) != 2 || servers[1] != "https://auth2.example.com" {
		t.Error("AddAuthorizationServer failed")
	}

	// Test adding duplicate server (should not add)
	handler.AddAuthorizationServer("https://auth2.example.com")
	if len(handler.GetMetadata().AuthorizationServers) != 2 {
		t.Error("AddAuthorizationServer should not add duplicates")
	}

	// Test RemoveAuthorizationServer
	handler.RemoveAuthorizationServer("https://auth2.example.com")
	if len(handler.GetMetadata().AuthorizationServers) != 1 {
		t.Error("RemoveAuthorizationServer failed")
	}

	// Test removing non-existent server (should not error)
	handler.RemoveAuthorizationServer("https://nonexistent.example.com")
	if len(handler.GetMetadata().AuthorizationServers) != 1 {
		t.Error("RemoveAuthorizationServer should handle non-existent servers gracefully")
	}
}

func TestResourceMetadataHandler_HttpMethods(t *testing.T) {
	// Test HTTP method handling for additional coverage
	handler := NewResourceMetadataHandler([]string{"https://auth.example.com"}, []string{"read"})

	// Test POST method (should return 405)
	req := httptest.NewRequest("POST", "/.well-known/oauth-protected-resource", nil)
	rr := httptest.NewRecorder()

	handler.HandleProtectedResourceMetadata(rr, req)
