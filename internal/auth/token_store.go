...
type TokenStore struct {
	...
	// wg tracks the cleanup goroutine so Close() can block until it exits.
	// Without this, callers would race with a still-running cleanup that
	// might touch the store after the caller assumed shutdown was complete.
	wg sync.WaitGroup
}
...
// startCleanupRoutine periodically cleans expired tokens. Calls wg.Done on
// exit so Close() can wait for the goroutine to actually finish.
...
// Close stops the cleanup routine and blocks until the goroutine has
// actually exited, so callers can safely drop the store without racing the
// background cleanup.
