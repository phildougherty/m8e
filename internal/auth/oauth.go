...
// VerifyCodeChallenge verifies a PKCE code challenge. Only S256 is accepted;
// the deprecated "plain" method (RFC 7636 §4.2, OAuth 2.1) is rejected.
// Comparison is constant-time even though the inputs are SHA-256 hashes —
// defense in depth and consistent with the broader credential-compare policy.
...
// GenerateCodeChallenge generates a PKCE code challenge. Only S256 is
// supported; the "plain" method is deprecated by OAuth 2.1 / RFC 7636 §7.2
// and is treated as an error.
func (v *DefaultCodeVerifier) GenerateCodeChallenge(verifier, method string) (string, error) {
	if method != "S256" {

		return "", fmt.Errorf("PKCE method %q is not supported; use S256", method)
	}
	hash := sha256.Sum256([]byte(verifier))
...
func NewAuthorizationServer(config *AuthorizationServerConfig, logger *logging.Logger) *AuthorizationServer {
	...
	if len(config.CodeChallengeMethodsSupported) == 0 {
		// OAuth 2.1 deprecates the "plain" PKCE method; advertise S256 only.
		config.CodeChallengeMethodsSupported = []string{"S256"}
	}
...
func (s *AuthorizationServer) ValidateClient(clientID, clientSecret string) (*OAuthClient, error) {
	...
	// For confidential clients, validate secret in constant time to avoid
	// leaking byte-by-byte timing information to a remote attacker.
