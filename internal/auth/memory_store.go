...
type MemoryTokenStore struct {
	...
	mu            sync.RWMutex
	cleanup       chan struct{}
	// wg tracks the background cleanup goroutine so Close() blocks until it
	// has actually exited. Without this, a caller could close the store and
	// then race with a still-running cleanup tick.
	wg sync.WaitGroup
}
...
// Close stops the cleanup routine and blocks until it has actually exited.
// Without the Wait, callers would race the still-running goroutine.
