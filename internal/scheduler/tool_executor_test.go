...
// TestToolExecutor_ExecuteStepWithContext_ToolFailureSurfaces is a
// regression test for the hallucination-class bug: when the MCP proxy
// returns a tool-level failure (Success=false), executeToolRequest returns
// a (result, nil) pair. ExecuteStepWithContext must NOT launder that into a
// "Succeeded" StepExecutionResult — doing so let workflow steps report
// success with empty output and let downstream LLM agents fabricate data.
func TestToolExecutor_ExecuteStepWithContext_ToolFailureSurfaces(t *testing.T) {
	logger := logr.Discard()
	...
	executor := NewToolExecutor(server.URL, "k", logger)
	result, err := executor.ExecuteStepWithContext(
		context.Background(),
		&StepExecutionContext{StepName: "s", WorkflowName: "w", WorkflowNamespace: "default"},
		"test-tool",
		map[string]interface{}{},
	)

	assert.NoError(t, err, "transport-level error should be nil; failure is in the result")
	assert.False(t, result.Success, "tool failure must produce Success=false")
	assert.Equal(t, "Failed", result.Status, "Status must reflect failure, not be hardcoded Succeeded")
	assert.Error(t, result.Error, "result.Error must be populated on failure")
	assert.Contains(t, result.Message, "upstream tool exploded")
}

// TestToolExecutor_ExecuteStepWithContext_ProxyUnreachable covers the other
// half of the bug: when the proxy is entirely unreachable, the step must
// fail loudly rather than silently produce a usable-looking result.
func TestToolExecutor_ExecuteStepWithContext_ProxyUnreachable(t *testing.T) {
	logger := logr.Discard()
	// Point at a closed port so the HTTP call fails outright.
	executor := NewToolExecutor("http://127.0.0.1:1", "k", logger)
