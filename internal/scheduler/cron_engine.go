...
type CronEngine struct {
	...
	mu      sync.RWMutex
	started bool
	// ctx is the engine lifecycle context, cancelled on Stop so in-flight
	// job retries can abort promptly instead of blocking on a fixed sleep.
	ctx    context.Context
	...
	retryLoop:
		for attempt := 1; attempt <= jobSpec.MaxRetries; attempt++ {
			...
			if attempt < jobSpec.MaxRetries {
				...
				select {
				case <-time.After(jobSpec.RetryDelay):
				case <-ctx.Done():
					e.logger.Info("Aborting job retry, engine stopping", "jobID", jobSpec.ID)
					lastErr = ctx.Err()
