...
// shortHash returns the first 8 hex chars of a SHA-256 digest. Used to fit
// long execution IDs / workflow names into Kubernetes' 63-char name limit.
// SHA-256 (truncated) is collision-resistant enough for naming; this replaces
// the previous MD5 use which was flagged by gosec.
...
func (kwe *K8sWorkflowExecutor) createWorkflowTaskRequest(workflowDef *crd.WorkflowDefinition, workflowName, executionID string) *task_scheduler.TaskRequest {
	...
	if len(executionID) > 20 {
		shortExecutionID = shortHash(executionID)
	}

	shortWorkflowName := workflowName
	if len(workflowName) > 20 {
		shortWorkflowName = shortHash(workflowName)
	}
...
func (kwe *K8sWorkflowExecutor) buildShellScript(workflowDef *crd.WorkflowDefinition) string {
	...
	if kwe.shouldEnableWorkspace(workflowDef) {
		...
		script += "echo 'Working directory set to workspace'\n"
	}
		...
		default:
			...
			if strings.HasPrefix(step.Tool, "mcp__") {
				...
				if len(parts) >= 3 {
					...
					script += fmt.Sprintf(`curl -s -X POST "${MCP_PROXY_URL:-http://matey-proxy.matey.svc.cluster.local:9876}/%s" \
...
func (kwe *K8sWorkflowExecutor) createWorkspaceVolume(workflowDef *crd.WorkflowDefinition, workflowName, executionID, shortWorkflowName, shortExecutionID string) task_scheduler.TaskVolumeConfig {
	...
	autoDelete := !workspace.RetainWorkspace && workspace.ReclaimPolicy != crd.WorkflowVolumeReclaimRetain
