...
func (tr *TemplateRegistry) substituteInMap(m map[string]interface{}, params map[string]interface{}) error {
	for key, value := range m {
		switch v := value.(type) {
		case string:
			if substituted, err := tr.substituteString(v, params); err == nil {
				m[key] = substituted
			}
		case map[string]interface{}:
			if err := tr.substituteInMap(v, params); err != nil {
				return err
			}
		}
	}
...
func (tr *TemplateRegistry) substituteString(s string, params map[string]interface{}) (string, error) {
	// Simple parameter substitution for now
	// In a more advanced implementation, this would use the template engine
	result := s
	for key, value := range params {
		...
		if str, ok := value.(string); ok {
			...
			_ = placeholder // Avoid unused variable error
			result = str
		}
	}
	return result, nil
