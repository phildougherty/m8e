...
func (r *MCPPostgresReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	...
	start := time.Now()
	defer func() {
		r.Metrics.RecordReconcile("MCPPostgres", time.Since(start), retErr)
	}()

	// Fetch the MCPPostgres instance
	postgres := &crd.MCPPostgres{}
	if found, err := fetchInstance(ctx, r.Client, req, postgres, "MCPPostgres"); err != nil || !found {
...
func (r *MCPPostgresReconciler) reconcileSecret(ctx context.Context, postgres *crd.MCPPostgres, log logr.Logger) error {
	...
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.Secret]{
		...
		NeedsUpdate: func(found, desired *corev1.Secret) bool {
			return string(found.Data["postgres-password"]) != postgres.Spec.Password
		},
		Mutate: func(found, desired *corev1.Secret) {
			found.Data["postgres-password"] = []byte(postgres.Spec.Password)
		},
	})
}
...
func (r *MCPPostgresReconciler) reconcilePVC(ctx context.Context, postgres *crd.MCPPostgres, log logr.Logger) error {
	...
	// PVCs are effectively immutable once bound, so this resource is create-only.
...
func (r *MCPPostgresReconciler) reconcileService(ctx context.Context, postgres *crd.MCPPostgres, log logr.Logger) error {
	...
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.Service]{
		...
		NeedsUpdate: func(found, desired *corev1.Service) bool {
			return found.Spec.Ports[0].Port != postgres.Spec.Port
		},
		Mutate: func(found, desired *corev1.Service) {
			found.Spec.Ports[0].Port = postgres.Spec.Port
			found.Spec.Ports[0].TargetPort = intstr.FromInt(int(postgres.Spec.Port))
		},
	})
}
...
func (r *MCPPostgresReconciler) reconcileDeployment(ctx context.Context, postgres *crd.MCPPostgres, log logr.Logger) error {
	...
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*appsv1.Deployment]{
		...
		NeedsUpdate: r.needsDeploymentUpdate,
