...
func (r *MCPProxyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	logger := log.FromContext(ctx)

	// Record reconcile metrics on the way out. retErr is a named return, so the
	// defer sees whatever error each early return assigns to it.
	start := time.Now()
	defer func() {
		r.Metrics.RecordReconcile("MCPProxy", time.Since(start), retErr)
	}()

	// Fetch the MCPProxy instance
	var mcpProxy crd.MCPProxy
	if found, err := fetchInstance(ctx, r.Client, req, &mcpProxy, "MCPProxy"); err != nil || !found {
	...
	if err := r.reconcileDeployment(ctx, &mcpProxy); err != nil {
		...
		if updateErr := r.Status().Update(ctx, &mcpProxy); updateErr != nil {
			logger.Error(updateErr, "Failed to update status after deployment reconciliation failure")
		}
		return ctrl.Result{}, err
	}
	...
	if err := r.reconcileService(ctx, &mcpProxy); err != nil {
		...
		mcpProxy.Status.Phase = crd.MCPProxyPhaseFailed
		if updateErr := r.Status().Update(ctx, &mcpProxy); updateErr != nil {
			logger.Error(updateErr, "Failed to update status after service reconciliation failure")
		}
		return ctrl.Result{}, err
	}
...
func (r *MCPProxyReconciler) reconcileDeployment(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	...
	// retry.RetryOnConflict re-fetches and re-applies on optimistic-lock conflicts
	// using a context-free exponential backoff; this replaces a hand-rolled
	// time.Sleep retry loop that blocked the reconcile goroutine.
	if !r.deploymentEqual(deployment, found) {
		if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			latest := &appsv1.Deployment{}
			if err := r.Get(ctx, client.ObjectKey{Name: deployment.Name, Namespace: deployment.Namespace}, latest); err != nil {
				return err
			}
...
func (r *MCPProxyReconciler) reconcileService(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	...
	if err := reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.Service]{
		Owner:   mcpProxy,
		Scheme:  r.Scheme,
		Desired: service,
		Empty:   func() *corev1.Service { return &corev1.Service{} },
		NeedsUpdate: func(found, desired *corev1.Service) bool {
			return found.Spec.Type != desired.Spec.Type || len(found.Spec.Ports) != len(desired.Spec.Ports)
		},
		Mutate: func(found, desired *corev1.Service) {
			found.Spec.Type = desired.Spec.Type
			found.Spec.Ports = desired.Spec.Ports
		},
	}); err != nil {
		logger.Error(err, "Failed to reconcile Service", "Service.Namespace", service.Namespace, "Service.Name", service.Name)
		return err
	}
...
func (r *MCPProxyReconciler) reconcileIngress(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	...
	if err := reconcileResource(ctx, r.Client, reconcileResourceOptions[*networkingv1.Ingress]{
		Owner:       mcpProxy,
		Scheme:      r.Scheme,
		Desired:     ingress,
		Empty:       func() *networkingv1.Ingress { return &networkingv1.Ingress{} },
		NeedsUpdate: alwaysUpdate[*networkingv1.Ingress],
		Mutate: func(found, desired *networkingv1.Ingress) {
			found.Spec = desired.Spec
			found.Annotations = desired.Annotations
		},
	}); err != nil {
		logger.Error(err, "Failed to reconcile Ingress", "Ingress.Namespace", ingress.Namespace, "Ingress.Name", ingress.Name)
		return err
	}
...
func (r *MCPProxyReconciler) getEnvVars(mcpProxy *crd.MCPProxy) []corev1.EnvVar {
	...
	// Add OAuth issuer override
	if mcpProxy.Spec.OAuth != nil && mcpProxy.Spec.OAuth.Issuer != "" {
		envVars = append(envVars, corev1.EnvVar{
			Name:  "OAUTH_ISSUER",
			Value: mcpProxy.Spec.OAuth.Issuer,
		})
	}
...
// reconcileConfigMap ensures the proxy uses the matey-config ConfigMap with full matey.yaml
func (r *MCPProxyReconciler) reconcileConfigMap(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	logger := log.FromContext(ctx)

	// Check if matey-config ConfigMap exists (created by controller manager)
	mateyConfig := &corev1.ConfigMap{}
	err := r.Get(ctx, client.ObjectKey{Name: "matey-config", Namespace: mcpProxy.Namespace}, mateyConfig)
	if err != nil {
		if errors.IsNotFound(err) {
			logger.Info("matey-config ConfigMap not found, creating basic proxy config")
			// Fall back to creating basic config if matey-config doesn't exist
			return r.reconcileBasicConfigMap(ctx, mcpProxy)
		}
		logger.Error(err, "Failed to get matey-config ConfigMap")
		return err
	}

	// Create/update proxy-specific ConfigMap that references matey-config
	configMap := &corev1.ConfigMap{
		...
		Data: r.buildProxyConfigFromMateyConfig(mcpProxy, mateyConfig),
	}

	if err := reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.ConfigMap]{
		Owner:   mcpProxy,
		Scheme:  r.Scheme,
		Desired: configMap,
		Empty:   func() *corev1.ConfigMap { return &corev1.ConfigMap{} },
		NeedsUpdate: func(found, desired *corev1.ConfigMap) bool {
			return !r.configMapEqual(desired, found)
		},
		Mutate: func(found, desired *corev1.ConfigMap) {
			found.Data = desired.Data
		},
	}); err != nil {
		logger.Error(err, "Failed to reconcile ConfigMap", "ConfigMap.Namespace", configMap.Namespace, "ConfigMap.Name", configMap.Name)
		return err
	}
...
func (r *MCPProxyReconciler) reconcilePodDisruptionBudget(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	...
	if err := reconcileResource(ctx, r.Client, reconcileResourceOptions[*policyv1.PodDisruptionBudget]{
		Owner:       mcpProxy,
		Scheme:      r.Scheme,
		Desired:     pdb,
		Empty:       func() *policyv1.PodDisruptionBudget { return &policyv1.PodDisruptionBudget{} },
		NeedsUpdate: alwaysUpdate[*policyv1.PodDisruptionBudget],
		Mutate: func(found, desired *policyv1.PodDisruptionBudget) {
			found.Spec = desired.Spec
		},
	}); err != nil {
		logger.Error(err, "Failed to reconcile PodDisruptionBudget", "PDB.Namespace", pdb.Namespace, "PDB.Name", pdb.Name)
		return err
	}
...
func (r *MCPProxyReconciler) reconcileCrossControllerIntegration(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	...
	endpoints := r.buildProxyEndpoints(mcpServers, mcpMemories)
...
func (r *MCPProxyReconciler) buildProxyConfig(mcpProxy *crd.MCPProxy) map[string]string {
	...
	// Add full YAML config file for OAuth and other advanced features
	if yamlConfig := r.buildYAMLConfig(mcpProxy); yamlConfig != "" {
		config["config.yaml"] = yamlConfig
	}

	return config
}

// buildProxyConfigFromMateyConfig builds proxy config using the full matey.yaml from matey-config
func (r *MCPProxyReconciler) buildProxyConfigFromMateyConfig(mcpProxy *crd.MCPProxy, mateyConfig *corev1.ConfigMap) map[string]string {
	config := map[string]string{
		"port":      fmt.Sprintf("%d", r.getPort(mcpProxy)),
		"namespace": mcpProxy.Namespace,
	}
	...
	// Use the full matey.yaml from matey-config ConfigMap if available
	if mateyYAML, exists := mateyConfig.Data["matey.yaml"]; exists {
		config["config.yaml"] = mateyYAML
	} else {
		// Fall back to building basic YAML config
		if yamlConfig := r.buildYAMLConfig(mcpProxy); yamlConfig != "" {
			config["config.yaml"] = yamlConfig
		}
	}
...
// reconcileBasicConfigMap creates a basic ConfigMap when matey-config is not available
func (r *MCPProxyReconciler) reconcileBasicConfigMap(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	logger := log.FromContext(ctx)
	...
	if err := reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.ConfigMap]{
		...
		logger.Error(err, "Failed to reconcile basic ConfigMap", "ConfigMap.Namespace", configMap.Namespace, "ConfigMap.Name", configMap.Name)
		return err
	}
...
func (r *MCPProxyReconciler) buildYAMLConfig(mcpProxy *crd.MCPProxy) string {
	// Only create YAML config if OAuth or other advanced features are enabled
	if mcpProxy.Spec.OAuth == nil || !mcpProxy.Spec.OAuth.Enabled {
		return ""
	}

	yamlConfig := fmt.Sprintf(`version: "1"
proxy_auth:
  enabled: %t`, mcpProxy.Spec.Auth != nil && mcpProxy.Spec.Auth.Enabled)

	if mcpProxy.Spec.Auth != nil && mcpProxy.Spec.Auth.APIKey != "" {
		yamlConfig += fmt.Sprintf(`
  api_key: %s`, mcpProxy.Spec.Auth.APIKey)
	}
...
oauth:
  enabled: true`
		...
		if mcpProxy.Spec.OAuth.Issuer != "" {
			yamlConfig += fmt.Sprintf(`
  issuer: %s`, mcpProxy.Spec.OAuth.Issuer)
		}

		// Add endpoints if any are configured
		if mcpProxy.Spec.OAuth.Endpoints.Authorization != "" ||
			mcpProxy.Spec.OAuth.Endpoints.Token != "" ||
			mcpProxy.Spec.OAuth.Endpoints.UserInfo != "" ||
			mcpProxy.Spec.OAuth.Endpoints.Revoke != "" ||
			mcpProxy.Spec.OAuth.Endpoints.Discovery != "" {
			yamlConfig += `
  endpoints:`
			if mcpProxy.Spec.OAuth.Endpoints.Authorization != "" {
				yamlConfig += fmt.Sprintf(`
    authorization: %s`, mcpProxy.Spec.OAuth.Endpoints.Authorization)
			}
			if mcpProxy.Spec.OAuth.Endpoints.Token != "" {
				yamlConfig += fmt.Sprintf(`
    token: %s`, mcpProxy.Spec.OAuth.Endpoints.Token)
			}
			if mcpProxy.Spec.OAuth.Endpoints.UserInfo != "" {
				yamlConfig += fmt.Sprintf(`
    userinfo: %s`, mcpProxy.Spec.OAuth.Endpoints.UserInfo)
			}
			if mcpProxy.Spec.OAuth.Endpoints.Revoke != "" {
				yamlConfig += fmt.Sprintf(`
    revoke: %s`, mcpProxy.Spec.OAuth.Endpoints.Revoke)
			}
			if mcpProxy.Spec.OAuth.Endpoints.Discovery != "" {
				yamlConfig += fmt.Sprintf(`
    discovery: %s`, mcpProxy.Spec.OAuth.Endpoints.Discovery)
			}
		}

		// Add tokens if any are configured
		if mcpProxy.Spec.OAuth.Tokens.AccessTokenTTL != "" ||
			mcpProxy.Spec.OAuth.Tokens.RefreshTokenTTL != "" ||
			mcpProxy.Spec.OAuth.Tokens.CodeTTL != "" ||
			mcpProxy.Spec.OAuth.Tokens.Algorithm != "" {
			yamlConfig += `
  tokens:`
			if mcpProxy.Spec.OAuth.Tokens.AccessTokenTTL != "" {
				yamlConfig += fmt.Sprintf(`
    access_token_ttl: %s`, mcpProxy.Spec.OAuth.Tokens.AccessTokenTTL)
			}
			if mcpProxy.Spec.OAuth.Tokens.RefreshTokenTTL != "" {
				yamlConfig += fmt.Sprintf(`
    refresh_token_ttl: %s`, mcpProxy.Spec.OAuth.Tokens.RefreshTokenTTL)
			}
			if mcpProxy.Spec.OAuth.Tokens.CodeTTL != "" {
				yamlConfig += fmt.Sprintf(`
    authorization_code_ttl: %s`, mcpProxy.Spec.OAuth.Tokens.CodeTTL)
			}
			if mcpProxy.Spec.OAuth.Tokens.Algorithm != "" {
				yamlConfig += fmt.Sprintf(`
    algorithm: %s`, mcpProxy.Spec.OAuth.Tokens.Algorithm)
			}
		}

		// Add security config (RequirePKCE is a bool, always has a value)
		yamlConfig += fmt.Sprintf(`
  security:
    require_pkce: %t`, mcpProxy.Spec.OAuth.Security.RequirePKCE)

		if len(mcpProxy.Spec.OAuth.GrantTypes) > 0 {
			yamlConfig += `
  grant_types:`
			for _, gt := range mcpProxy.Spec.OAuth.GrantTypes {
				yamlConfig += fmt.Sprintf(`
    - %s`, gt)
			}
		}

		if len(mcpProxy.Spec.OAuth.ResponseTypes) > 0 {
			yamlConfig += `
  response_types:`
			for _, rt := range mcpProxy.Spec.OAuth.ResponseTypes {
				yamlConfig += fmt.Sprintf(`
    - %s`, rt)
			}
		}

		if len(mcpProxy.Spec.OAuth.ScopesSupported) > 0 {
			yamlConfig += `
  scopes_supported:`
			for _, scope := range mcpProxy.Spec.OAuth.ScopesSupported {
				yamlConfig += fmt.Sprintf(`
    - %s`, scope)
			}
		}
...
func (r *MCPProxyReconciler) shouldIncludeService(mcpProxy *crd.MCPProxy, svc *discovery.ServiceEndpoint) bool {
	...
	// Match against the label selector, if one is configured. The CRD's
	// LabelSelector mirrors metav1.LabelSelector, so it is converted and
	// evaluated with the standard apimachinery selector machinery against the
	// discovered service's labels.
	if selector.LabelSelector != nil {
		return matchesLabelSelector(selector.LabelSelector, svc.Labels)
	}

	return true
}

// matchesLabelSelector reports whether the given label set satisfies the
// supplied CRD label selector. A nil or empty selector matches everything; a
// selector that fails to parse matches nothing (so a misconfigured selector
// does not silently widen discovery).
func matchesLabelSelector(sel *crd.LabelSelector, lbls map[string]string) bool {
	if sel == nil {
		return true
	}

	metaSel := &metav1.LabelSelector{MatchLabels: sel.MatchLabels}
	...
	parsed, err := metav1.LabelSelectorAsSelector(metaSel)
	if err != nil {
		return false
	}
...
func (r *MCPProxyReconciler) buildProxyEndpoints(mcpServers *crd.MCPServerList, mcpMemories *crd.MCPMemoryList) []crd.ProxyEndpoint {
