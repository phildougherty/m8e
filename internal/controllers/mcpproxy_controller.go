...
func (r *MCPProxyReconciler) reconcileBasicConfigMap(ctx context.Context, mcpProxy *crd.MCPProxy) error {
	...
	err := r.Get(ctx, client.ObjectKey{Name: configMap.Name, Namespace: configMap.Namespace}, found)
	if err != nil && errors.IsNotFound(err) {
		logger.Info("Creating a new basic ConfigMap", "ConfigMap.Namespace", configMap.Namespace, "ConfigMap.Name", configMap.Name)
		if err := r.Create(ctx, configMap); err != nil {
			logger.Error(err, "Failed to create new basic ConfigMap", "ConfigMap.Namespace", configMap.Namespace, "ConfigMap.Name", configMap.Name)
			return err
		}
		return nil
	} else if err != nil {
		logger.Error(err, "Failed to get basic ConfigMap")
		return err
	}
	...
	if !r.configMapEqual(configMap, found) {
		found.Data = configMap.Data
		logger.Info("Updating basic ConfigMap", "ConfigMap.Namespace", found.Namespace, "ConfigMap.Name", found.Name)
		if err := r.Update(ctx, found); err != nil {
			logger.Error(err, "Failed to update basic ConfigMap", "ConfigMap.Namespace", found.Namespace, "ConfigMap.Name", found.Name)
			return err
		}
	}
