...
func (r *MCPMemoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	...
	start := time.Now()
	defer func() {
		r.Metrics.RecordReconcile("MCPMemory", time.Since(start), retErr)
	}()

	// Fetch the MCPMemory instance
	memory := &crd.MCPMemory{}
	if found, err := fetchInstance(ctx, r.Client, req, memory, "MCPMemory"); err != nil || !found {
...
func (r *MCPMemoryReconciler) reconcileRunning(ctx context.Context, memory *crd.MCPMemory) (ctrl.Result, error) {
	...
	if memory.Spec.PostgresEnabled {
		...
		postgresHealthy, err := r.isPostgresReady(ctx, memory)
		if err != nil {
			logger.Error(err, "Failed to check PostgreSQL readiness")
			r.updateCondition(memory, crd.MCPMemoryConditionPostgresReady, metav1.ConditionFalse, "PostgresCheckFailed", "Failed to check PostgreSQL status: "+err.Error())
			memory.Status.PostgresStatus = "unknown"
...
func (r *MCPMemoryReconciler) reconcileSecret(ctx context.Context, memory *crd.MCPMemory) error {
	...
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.Secret]{
		Owner:       memory,
		Scheme:      r.Scheme,
		Desired:     secret,
		Empty:       func() *corev1.Secret { return &corev1.Secret{} },
		NeedsUpdate: alwaysUpdate[*corev1.Secret],
		Mutate: func(found, desired *corev1.Secret) {
...
func (r *MCPMemoryReconciler) reconcileService(ctx context.Context, memory *crd.MCPMemory) error {
	...
	// Service is create-only here; spec changes are not reconciled in place.
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.Service]{
		Owner:   memory,
		Scheme:  r.Scheme,
		Desired: service,
...
func (r *MCPMemoryReconciler) reconcileDeployment(ctx context.Context, memory *crd.MCPMemory) error {
	...
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*appsv1.Deployment]{
		Owner:       memory,
		Scheme:      r.Scheme,
		Desired:     deployment,
		Empty:       func() *appsv1.Deployment { return &appsv1.Deployment{} },
		NeedsUpdate: alwaysUpdate[*appsv1.Deployment],
		Mutate: func(found, desired *appsv1.Deployment) {
...
func (r *MCPMemoryReconciler) reconcilePostgresPVC(ctx context.Context, memory *crd.MCPMemory) error {
	...
	// PVC is create-only; it is never updated once it exists.
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.PersistentVolumeClaim]{
		Owner:   memory,
		Scheme:  r.Scheme,
		Desired: pvc,
		Empty:   func() *corev1.PersistentVolumeClaim { return &corev1.PersistentVolumeClaim{} },
	})
}
...
func (r *MCPMemoryReconciler) reconcilePostgresService(ctx context.Context, memory *crd.MCPMemory) error {
	...
	// PostgreSQL service is create-only; it is never updated once it exists.
	return reconcileResource(ctx, r.Client, reconcileResourceOptions[*corev1.Service]{
		Owner:   memory,
		Scheme:  r.Scheme,
		Desired: service,
...
// buildMemoryEnvVars creates environment variables for the memory container
func (r *MCPMemoryReconciler) buildMemoryEnvVars(memory *crd.MCPMemory) []corev1.EnvVar {
	env := []corev1.EnvVar{
...
func (r *MCPMemoryReconciler) buildMemoryContainer(memory *crd.MCPMemory) corev1.Container {
	...
	container := corev1.Container{
		...
		Env:             r.buildMemoryEnvVars(memory),
	...
	r.applyMemoryResources(&container, memory)
	return container
}

// applyMemoryResources applies resource requirements to the container
func (r *MCPMemoryReconciler) applyMemoryResources(container *corev1.Container, memory *crd.MCPMemory) {
...
func (r *MCPMemoryReconciler) buildMemoryPodSpec(memory *crd.MCPMemory) corev1.PodSpec {
	container := r.buildMemoryContainer(memory)
