...
func (r *MCPTaskSchedulerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	...
	start := time.Now()
	defer func() {
		r.Metrics.RecordReconcile("MCPTaskScheduler", time.Since(start), retErr)
	}()

	// Fetch the MCPTaskScheduler instance
	taskScheduler := &crd.MCPTaskScheduler{}
	if found, err := fetchInstance(ctx, r.Client, req, taskScheduler, "MCPTaskScheduler"); err != nil || !found {
...
func (r *MCPTaskSchedulerReconciler) reconcileService(ctx context.Context, taskScheduler *crd.MCPTaskScheduler) error {
	...
	// Services are mostly immutable here, so this resource is create-only.
