...
func (r *MCPServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	...
	start := time.Now()
	defer func() {
		r.Metrics.RecordReconcile("MCPServer", time.Since(start), retErr)
	}()

	// Fetch the MCPServer instance
	var mcpServer mcpv1.MCPServer
	if found, err := fetchInstance(ctx, r.Client, req, &mcpServer, "MCPServer"); err != nil || !found {
	...
	// Capture the phase observed at the start of this reconcile so updateStatus
	// can detect a transition into/out of Running and emit lifecycle metrics.
	priorPhase := mcpServer.Status.Phase
	...
	if err := r.updateStatus(ctx, &mcpServer, deployment, service, priorPhase); err != nil {
		logger.Error(err, "Failed to update MCPServer status")
		return ctrl.Result{}, err
	}

	logger.V(1).Info("Successfully reconciled MCPServer", "name", mcpServer.Name)
	return ctrl.Result{RequeueAfter: config.DefaultRequeueAfterError}, nil
}
...
// It uses controllerutil.CreateOrUpdate so the desired spec is reapplied on
// every reconcile, then returns the live Deployment for status derivation.
...
// buildLabels creates the standard labels for MCP server resources
...
// getReplicas returns the replica count for the deployment
func (r *MCPServerReconciler) getReplicas(mcpServer *mcpv1.MCPServer) int32 {
	replicas := int32(1)
...
func (r *MCPServerReconciler) buildContainer(mcpServer *mcpv1.MCPServer) corev1.Container {
	...
	r.setContainerCommand(&container, mcpServer)
	r.setContainerEnv(&container, mcpServer)
	r.setContainerPorts(&container, mcpServer)
	r.setContainerResources(&container, mcpServer)
	r.setContainerSecurity(&container, mcpServer)
	r.setContainerVolumes(&container, mcpServer)

	return container
}

// setContainerCommand sets command and args for the container
func (r *MCPServerReconciler) setContainerCommand(container *corev1.Container, mcpServer *mcpv1.MCPServer) {
...
// setContainerEnv sets environment variables for the container
func (r *MCPServerReconciler) setContainerEnv(container *corev1.Container, mcpServer *mcpv1.MCPServer) {
...
// setContainerPorts sets container ports
func (r *MCPServerReconciler) setContainerPorts(container *corev1.Container, mcpServer *mcpv1.MCPServer) {
...
// setContainerResources sets resource requirements for the container
func (r *MCPServerReconciler) setContainerResources(container *corev1.Container, mcpServer *mcpv1.MCPServer) {
...
// setContainerSecurity sets security context for the container
func (r *MCPServerReconciler) setContainerSecurity(container *corev1.Container, mcpServer *mcpv1.MCPServer) {
	...
	if len(mcpServer.Spec.Security.CapAdd) > 0 || len(mcpServer.Spec.Security.CapDrop) > 0 {
		...
		if len(mcpServer.Spec.Security.CapAdd) > 0 {
			for _, cap := range mcpServer.Spec.Security.CapAdd {
				container.SecurityContext.Capabilities.Add = append(container.SecurityContext.Capabilities.Add, corev1.Capability(cap))
			}
		}
		if len(mcpServer.Spec.Security.CapDrop) > 0 {
...
// setContainerVolumes sets volume mounts for the container
func (r *MCPServerReconciler) setContainerVolumes(container *corev1.Container, mcpServer *mcpv1.MCPServer) {
...
func (r *MCPServerReconciler) buildDeploymentSpec(mcpServer *mcpv1.MCPServer) appsv1.DeploymentSpec {
	...
	replicas := r.getReplicas(mcpServer)
...
func (r *MCPServerReconciler) updateStatus(ctx context.Context, mcpServer *mcpv1.MCPServer, deployment *appsv1.Deployment, service *corev1.Service, priorPhase mcpv1.MCPServerPhase) error {
	...
	// Emit server lifecycle metrics on an observed phase transition. The
	// controller's only clean running/stopped signal is the status phase it
	// derives from the Deployment's ready replicas, so a start is recorded when
	// the phase first becomes Running and a stop when it leaves Running.
	if newStatus.Phase != priorPhase {
		switch {
		case newStatus.Phase == mcpv1.MCPServerPhaseRunning:
			r.Metrics.RecordServerStart(mcpServer.Name)
		case priorPhase == mcpv1.MCPServerPhaseRunning:
			r.Metrics.RecordServerStop(mcpServer.Name)
		}
	}
