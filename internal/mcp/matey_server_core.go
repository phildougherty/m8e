...
func (m *MateyMCPServer) executeAgent(ctx context.Context, arguments map[string]interface{}) (*ToolResult, error) {
	fmt.Printf("DEBUG: executeAgent called at %s with arguments: %+v\n", time.Now().Format("15:04:05"), arguments)
	fmt.Printf("DEBUG: Arguments type: %T, length: %d\n", arguments, len(arguments))
	for key, value := range arguments {
		fmt.Printf("DEBUG: Key='%s', Value='%v', ValueType=%T\n", key, value, value)
	}
	...
	if objective == "" {
		...
		fmt.Printf("DEBUG: executeAgent failed - %s\n", errorMsg)
		return &ToolResult{
	...
	fmt.Printf("   │ DEBUG: Context timeout set to %d seconds\n", timeoutSeconds)
	fmt.Printf("   │ DEBUG: Starting execute_agent at %s\n", time.Now().Format("15:04:05"))
...
func (m *MateyMCPServer) executeRealLLMAgent(ctx context.Context, objective, systemPrompt string, progress *AgentProgress, result *AgentResult, maxTurns int) int {
	...
	for turn := 0; turn < maxTurns && toolsExecuted < maxTools; turn++ {
		...
		select {
		case <-ctx.Done():
			fmt.Printf("   │ DEBUG: Context already cancelled before LLM call: %v\n", ctx.Err())
			break
		default:
			// Continue
		}
		...
		fmt.Printf("   │ DEBUG: About to call openrouter-gateway at %s\n", time.Now().Format("15:04:05"))
		...
		if err != nil {
			...
			if ctx.Err() != nil {
				if ctx.Err() == context.DeadlineExceeded {
					...
					fmt.Printf("   │ DEBUG: Context error detected: %v\n", ctx.Err())
		...
		fmt.Printf("   │ DEBUG: OpenRouter gateway call completed at %s\n", time.Now().Format("15:04:05"))
...
func (m *MateyMCPServer) callOpenRouterGateway(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
	...
	fmt.Printf("   │ DEBUG: Creating HTTP request at %s\n", time.Now().Format("15:04:05"))
	...
	fmt.Printf("   │ DEBUG: Making HTTP request to openrouter-gateway at %s\n", time.Now().Format("15:04:05"))
	...
	fmt.Printf("   │ DEBUG: HTTP request completed at %s\n", time.Now().Format("15:04:05"))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		fmt.Printf("   │ DEBUG: HTTP error response body: %s\n", string(body))
		return nil, fmt.Errorf("HTTP %d: %s", resp.StatusCode, string(body))
	}
	
	// Read and parse response body  
	fmt.Printf("   │ DEBUG: Reading response body at %s\n", time.Now().Format("15:04:05"))
	...
	fmt.Printf("   │ DEBUG: Parsing JSON response at %s\n", time.Now().Format("15:04:05"))
	...
	fmt.Printf("   │ DEBUG: JSON parsing completed successfully at %s\n", time.Now().Format("15:04:05"))
...
// buildAvailableToolsForLLM converts MCP tools to AI provider tool format
func (m *MateyMCPServer) buildAvailableToolsForLLM() []ai.Function {
	mcpTools := m.GetTools()
	var aiFunctions []ai.Function
	...
	for _, mcpTool := range mcpTools {
		...
		// Convert MCP tool schema to AI provider format
		aiFunction := ai.Function{
			Name:        mcpTool.Name,
			Description: mcpTool.Description,
			Parameters:  mcpTool.InputSchema.(map[string]interface{}),
		}
		aiFunctions = append(aiFunctions, aiFunction)
	}
...
// tryExecuteTool attempts to execute a tool and updates progress, returns true if successful
func (m *MateyMCPServer) tryExecuteTool(ctx context.Context, toolName string, args map[string]interface{}, progress *AgentProgress, result *AgentResult) bool {
	startTime := time.Now()
	
	// Execute the tool using the same logic as ExecuteTool
	toolResult, err := m.ExecuteTool(ctx, toolName, args)
	...
	if err != nil {
		...
		// Extract meaningful summary from the tool result
		summary := m.extractToolSummary(toolResult, toolName)
		progress.UpdateDisplay(ToolCallSummary{
			Name:     toolName,
			Status:   "✓",
			Summary:  summary,
		...
		if result.StructuredData == nil {
			result.StructuredData = make(map[string]interface{})
		}
		if structuredData, ok := result.StructuredData.(map[string]interface{}); ok {
			structuredData[toolName+"_result"] = toolResult
		}
...
// shouldExecuteTools determines if the LLM response indicates a tool should be executed
func (m *MateyMCPServer) shouldExecuteTools(response string) bool {
	// Look for tool execution patterns in the response
	toolKeywords := []string{
		"tool:", "call:", "execute:", "run:", "use:",
		"matey_ps", "matey_logs", "get_cluster_state", "memory_", "search_",
	}
...
// parseToolFromResponse extracts tool name and arguments from LLM response
func (m *MateyMCPServer) parseToolFromResponse(response string) (string, map[string]interface{}) {
	// Simple parsing - look for patterns like "tool: toolname" or "call: toolname(args)"
	lines := strings.Split(response, "\n")
	
	for _, line := range lines {
		line = strings.TrimSpace(strings.ToLower(line))
		
		// Pattern: "tool: toolname" or "call: toolname"
		if strings.HasPrefix(line, "tool:") || strings.HasPrefix(line, "call:") {
			...
			if len(parts) >= 2 {
				...
				// Basic argument parsing (could be enhanced)
				args := make(map[string]interface{})
				return toolName, args
			}
		}
		
		// Pattern: "execute toolname" or "run toolname"
		if strings.HasPrefix(line, "execute ") || strings.HasPrefix(line, "run ") {
			parts := strings.Fields(line)
			if len(parts) >= 2 {
				toolName := parts[1]
				toolName = strings.Trim(toolName, "()[]{}.,")
		...
		// Pattern: direct tool mention for common tools
		commonTools := []string{
			"matey_ps", "matey_logs", "get_cluster_state", "memory_status",
			"memory_health_check", "task_scheduler_status", "search_in_files",
		}
...
// executePatternMatchingAgent executes using pattern matching as a fallback
func (m *MateyMCPServer) executePatternMatchingAgent(ctx context.Context, objective string, progress *AgentProgress, result *AgentResult, maxTurns int) int {
	fmt.Printf("   │ Using pattern matching fallback for objective: %s\n", objective)
	...
	// Execute tools based on objective patterns
	if strings.Contains(objectiveLower, "cluster") || strings.Contains(objectiveLower, "status") {
		if toolResult, err := m.ExecuteTool(ctx, "get_cluster_state", map[string]interface{}{}); err == nil {
			summary := m.extractToolSummary(toolResult, "get_cluster_state")
			fmt.Printf("   │ ✓ get_cluster_state - %s\n", summary)
			progress.UpdateDisplay(ToolCallSummary{
			...
			result.ToolsUsed["get_cluster_state"]++
			toolsExecuted++
		}
	}
	
	if strings.Contains(objectiveLower, "memory") {
		if toolResult, err := m.ExecuteTool(ctx, "memory_status", map[string]interface{}{}); err == nil {
			summary := m.extractToolSummary(toolResult, "memory_status")
			fmt.Printf("   │ ✓ memory_status - %s\n", summary)
			progress.UpdateDisplay(ToolCallSummary{
				Name:     "memory_status",
				Status:   "✓",
				Summary:  summary,
			...
			result.ToolsUsed["memory_status"]++
			toolsExecuted++
		}
	}
	
	if strings.Contains(objectiveLower, "service") || strings.Contains(objectiveLower, "ps") {
		if toolResult, err := m.ExecuteTool(ctx, "matey_ps", map[string]interface{}{}); err == nil {
			summary := m.extractToolSummary(toolResult, "matey_ps")
			fmt.Printf("   │ ✓ matey_ps - %s\n", summary)
			progress.UpdateDisplay(ToolCallSummary{
				Name:     "matey_ps",
				Status:   "✓",
				Summary:  summary,
			...
			result.ToolsUsed["matey_ps"]++
			toolsExecuted++
		}
	}
	
	fmt.Printf("   │ Pattern matching complete: %d tools executed\n", toolsExecuted)
	return toolsExecuted
}
