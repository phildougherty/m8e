...
// workflowTools owns the workflow lifecycle tools (create/list/execute/delete)
// and workflow execution-history reporting (workflow_logs). Workflows are
// stored as entries in the MCPTaskScheduler CRD's spec.
type workflowTools struct {
	deps clusterDeps
}

func newWorkflowTools(deps clusterDeps) *workflowTools {
...
func (w *workflowTools) createWorkflow(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
	...
	// Workflows are persisted as entries in the MCPTaskScheduler CRD, so a
	// Kubernetes client is mandatory — there is no binary/local mode that can
	// mutate a cluster resource.
	if !w.deps.useK8sClient() {
		return w.errNoK8sClient("create")
	}

	result, err := w.addWorkflowToTaskScheduler(ctx, workflowDef)
	if err != nil {
		return &ToolResult{
...
// errNoK8sClient returns a uniform, honest error for workflow operations that
// require a Kubernetes client. Workflows live inside the MCPTaskScheduler CRD;
// without a cluster connection there is nothing to create, list, or trigger.
func (w *workflowTools) errNoK8sClient(operation string) (*ToolResult, error) {
	msg := fmt.Sprintf("cannot %s workflow: no Kubernetes client available. "+
		"Workflows are stored in the MCPTaskScheduler CRD and require a cluster connection "+
		"(run inside the cluster or provide a valid kubeconfig).", operation)

	return &ToolResult{
		Content: []Content{{Type: "text", Text: msg}},
		IsError: true,
	}, fmt.Errorf("%s workflow requires a kubernetes client", operation)
}

// addWorkflowToTaskScheduler adds a workflow to an existing MCPTaskScheduler
func (w *workflowTools) addWorkflowToTaskScheduler(ctx context.Context, workflowDef map[string]interface{}) (*ToolResult, error) {
	...
	err := w.deps.k8sClient.Get(ctx, client.ObjectKey{
		Name:      taskSchedulerName,
		Namespace: w.deps.namespace,
	}, &taskScheduler)
	...
	err = w.deps.k8sClient.Update(ctx, &taskScheduler)
	if err != nil {
		return &ToolResult{
...
func (w *workflowTools) listWorkflows(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
	if !w.deps.useK8sClient() {
		return w.errNoK8sClient("list")
	}
	...
	err := w.deps.k8sClient.List(ctx, &taskSchedulers, listOpts)
	if err != nil {
		return &ToolResult{
	...
	return w.formatWorkflowsList(allWorkflows, outputFormat)
}

// formatWorkflowsList formats workflow list output
func (w *workflowTools) formatWorkflowsList(workflows []map[string]interface{}, outputFormat string) (*ToolResult, error) {
	switch outputFormat {
	case "json":
...
func (w *workflowTools) deleteWorkflow(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
	name, ok := args["name"].(string)
	if !ok || name == "" {
		return &ToolResult{
			Content: []Content{{Type: "text", Text: "name is required"}},
	...
	if !w.deps.useK8sClient() {
		return w.errNoK8sClient("delete")
	}
...
func (w *workflowTools) executeWorkflow(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
	name, ok := args["name"].(string)
	if !ok || name == "" {
		return &ToolResult{
			Content: []Content{{Type: "text", Text: "name is required"}},
	...
	if !w.deps.useK8sClient() {
		return w.errNoK8sClient("execute")
	}

	// Manual execution is triggered by annotating the MCPTaskScheduler CRD;
	// the task-scheduler controller's handleManualExecutions reconcile path
	// picks up the annotation and runs the workflow via the WorkflowScheduler.
	return w.executeWorkflowViaMCP(ctx, name, args)
}

// executeWorkflowViaMCP executes a workflow by triggering it through the MCPTaskScheduler CRD
func (w *workflowTools) executeWorkflowViaMCP(ctx context.Context, workflowName string, args map[string]interface{}) (*ToolResult, error) {
	...
	resultText += "🔄 Status: Queued for execution by task-scheduler controller\n"
...
func (w *workflowTools) workflowLogs(ctx context.Context, args map[string]interface{}) (*ToolResult, error) {
	name, ok := args["name"].(string)
	if !ok || name == "" {
		return &ToolResult{
			Content: []Content{{Type: "text", Text: "name is required"}},
	...
	if !w.deps.useK8sClient() {
		return w.errNoK8sClient("read logs for")
	}

	return w.getWorkflowExecutionHistory(ctx, name, executionID, step, int(tailLines))
}

// getWorkflowExecutionHistory retrieves comprehensive workflow execution history from Kubernetes
func (w *workflowTools) getWorkflowExecutionHistory(ctx context.Context, workflowName, executionID, step string, tailLines int) (*ToolResult, error) {
	...
	if executionID != "" {
		for _, execution := range matchingExecutions {
			if execution.ID == executionID {
				return w.formatDetailedExecution(execution, step)
			}
		}
		return &ToolResult{
	...
	return w.formatExecutionHistory(executions, workflowName, &taskScheduler.Status)
}

// formatDetailedExecution formats detailed information for a single execution
func (w *workflowTools) formatDetailedExecution(execution crd.WorkflowExecution, specificStep string) (*ToolResult, error) {
	var result strings.Builder

	result.WriteString("📊 **Workflow Execution Details**\n\n")
	result.WriteString(fmt.Sprintf("**Workflow**: %s\n", execution.WorkflowName))
	result.WriteString(fmt.Sprintf("**Execution ID**: %s\n", execution.ID))
	result.WriteString(fmt.Sprintf("**Status**: %s %s\n", w.getStatusIcon(string(execution.Phase)), execution.Phase))
...
func (w *workflowTools) formatExecutionHistory(executions []crd.WorkflowExecution, workflowName string, status *crd.MCPTaskSchedulerStatus) (*ToolResult, error) {
	var result strings.Builder

	result.WriteString(fmt.Sprintf("📊 **Workflow Execution History: %s**\n\n", workflowName))

	// Calculate workflow-specific statistics
	workflowStats := w.calculateWorkflowStats(executions)
	...
	if len(executions) == 0 {
		...
		for i, execution := range executions {
			statusIcon := w.getStatusIcon(string(execution.Phase))
			...
			// Step summary
			if len(execution.StepResults) > 0 {
				stepSummary := w.getStepSummary(execution.StepResults)
...
func (w *workflowTools) calculateWorkflowStats(executions []crd.WorkflowExecution) WorkflowStats {
	stats := WorkflowStats{
...
func (w *workflowTools) getStatusIcon(phase string) string {
	switch phase {
	case "Succeeded":
		return "SUCCESS"
	case "Failed":
...
func (w *workflowTools) getStepSummary(stepResults map[string]crd.StepResult) string {
...
func (w *workflowTools) formatStepOutput(output interface{}) string {
	if output == nil {
		return "(no output)"
	}
