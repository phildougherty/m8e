...
func (m *MateyMCPServer) GetTools() []Tool {
	return []Tool{
		...
		{
			Name:        "execute_bash",
			Description: "Execute a bash command on the matey host. By default only allowlisted binaries (kubectl, ls, grep, git, ...) are permitted and command substitution is rejected; the environment is scrubbed of credential-looking variables. Operators can widen this via MATEY_BASH_ALLOWLIST or disable filtering with MATEY_BASH_MODE=unrestricted. This runs real commands and can change real state.",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
		...
		// Context Mentions (2 tools)
		{
			...
			Description: "Parse @mentions out of a chunk of text and resolve each one against its real backend (k8s for @problems / @logs / @workflow, tree-sitter for @def, the memory store for @memory, the filesystem for @file / @directory). Returns a JSON payload with the resolved mentions and their content.",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"text": map[string]interface{}{
						"type":        "string",
						"description": "Text containing one or more @mentions to resolve",
					},
				},
		...
		{
			Name:        "expand_mentions",
			Description: "Return the input text with every @mention replaced inline by its resolved content. Useful when you want a single self-contained prompt instead of structured per-mention data.",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"text": map[string]interface{}{
						"type":        "string",
						"description": "Text containing one or more @mentions to expand inline",
					},
				},
				"required": []string{"text"},
			},
