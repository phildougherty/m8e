...
type EditFileRequest struct {
	...
	Path        string          `json:"path,omitempty"` // Alternative field name for compatibility
	Content     string          `json:"content,omitempty"`
	Edits       []EditOperation `json:"edits,omitempty"` // Multiple edit operations
...
func (et *EnhancedTools) EditFile(ctx context.Context, request EditFileRequest) (*EditFileResponse, error) {
	...
	// Handle path compatibility - use Path if FilePath is empty
	filePath := request.FilePath
	if filePath == "" && request.Path != "" {
		filePath = request.Path
		request.FilePath = filePath // Normalize for response
		response.FilePath = filePath
	}

	// Validate file path
	if filePath == "" {
		response.Error = "file_path or path is required"
		return response, fmt.Errorf("file_path or path is required")
	}
	...
	if et.fileEditor != nil {
		originalContent, err = et.fileEditor.ReadFile(filePath)
		if err != nil && !request.CreateFile {
			response.Error = fmt.Sprintf("failed to read file: %v", err)
			return response, err
		}
	} else {
		// Fallback to standard file operations when fileEditor is nil
	...
	if request.DiffContent != "" {
		...
		response.Error = "either content, diff_content, or edits must be provided"
		return response, fmt.Errorf("either content, diff_content, or edits must be provided")
	}

	// Generate simple diff preview (visual package removed)
	if et.config.EnableVisualDiff && originalContent != "" {
		response.DiffPreview = fmt.Sprintf("Changes to be applied to %s\nOriginal length: %d bytes\nNew length: %d bytes",
			request.FilePath, len(originalContent), len(newContent))
	}

	// Auto-approve since visual diff is no longer available
	...
	if response.Applied {
		if et.fileEditor != nil {
			...
			// Fallback to standard file operations when fileEditor is nil
			err = os.WriteFile(request.FilePath, []byte(newContent), 0644)
			if err != nil {
				response.Error = fmt.Sprintf("failed to write file: %v", err)
				return response, err
			}
...
func (et *EnhancedTools) generateFilePreview(filePath, content string) string {
	...
	fmt.Fprintf(&preview, "File: %s\n", filePath)
	fmt.Fprintf(&preview, "Size: %d bytes\n", len(content))
	fmt.Fprintf(&preview, "Lines: %d\n", strings.Count(content, "\n")+1)
