...
func (m *K8sManager) Restart() error {
	...
	// Wait for the MCPTaskScheduler resource to actually be deleted before
	...
	_ = wait.PollUntilContextTimeout(ctx, 200*time.Millisecond, 10*time.Second, true, func(ctx context.Context) (bool, error) {
		...
		getErr := m.client.Get(ctx, types.NamespacedName{Name: "task-scheduler", Namespace: m.namespace}, ts)
