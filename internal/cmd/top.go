...
// ServerInfo contains detailed information about a server. The data-gathering
// lives in the service layer; this alias keeps the TUI rendering code stable.
type ServerInfo = service.ServerInfo
...
func runTop(configFile string, refreshRate time.Duration) error {
	// Create composer
	composer, err := compose.NewK8sComposer(configFile, constants.MateyNamespace)
	if err != nil {
	...
	// Initialize model
	model := TopModel{
		stats:       service.NewClusterStats(composer, k8sClient, constants.MateyNamespace),
...
func parseMemoryValue(memory string) int64 {
	...
	if strings.HasSuffix(memory, "Mi") {
		if val, err := fmt.Sscanf(memory, "%dMi", new(int64)); err == nil && val == 1 {
			var mem int64
			if _, err := fmt.Sscanf(memory, "%dMi", &mem); err == nil {
	...
	if strings.HasSuffix(memory, "Gi") {
		if val, err := fmt.Sscanf(memory, "%dGi", new(int64)); err == nil && val == 1 {
			var mem int64
			if _, err := fmt.Sscanf(memory, "%dGi", &mem); err == nil {
	...
	if strings.HasSuffix(memory, "Ki") {
		if val, err := fmt.Sscanf(memory, "%dKi", new(int64)); err == nil && val == 1 {
			var mem int64
			if _, err := fmt.Sscanf(memory, "%dKi", &mem); err == nil {
	...
	if val, err := fmt.Sscanf(memory, "%d", new(int64)); err == nil && val == 1 {
		var mem int64
		if _, err := fmt.Sscanf(memory, "%d", &mem); err == nil {
			return mem
		}
	}
...
func parseCPUValue(cpu string) int64 {
	...
	if strings.HasSuffix(cpu, "m") {
		if val, err := fmt.Sscanf(cpu, "%dm", new(int64)); err == nil && val == 1 {
			var cpuVal int64
			if _, err := fmt.Sscanf(cpu, "%dm", &cpuVal); err == nil {
...
// getServerInfo gets detailed information about all servers by delegating to
// the ClusterStats service.
func (m *TopModel) getServerInfo() ([]ServerInfo, error) {
	return m.stats.ServerInfos(context.Background())
}
