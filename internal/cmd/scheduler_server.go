  ...
  matey scheduler-server --namespace prod   # Run in specific namespace`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.TaskScheduler != nil && cfg.TaskScheduler.DatabaseURL != "" {
				...
				// Try default PostgreSQL connection. The password is sourced
				// from POSTGRES_PASSWORD; if it is unset we REFUSE to fall
				// back to a plaintext "postgres:password" default — a deploy
				// that forgot to wire the Secret would otherwise either fail
				// mysteriously or, worse, succeed against a dev DB.
				defaultDatabaseURL, dsnErr := service.BuildDefaultPostgresDSN("matey")
				if dsnErr != nil {
					logger.Error("Cannot construct default workflow store DSN: %v", dsnErr)
					logger.Error("Set POSTGRES_PASSWORD (e.g. via valueFrom.secretKeyRef on a Kubernetes Secret) or set task_scheduler.database_url explicitly.")

					return fmt.Errorf("scheduler-server: %w", dsnErr)
				}
