...
func init() {
	...
	if err := schedulerExecuteWorkflowCmd.MarkFlagRequired("workflow-name"); err != nil {
		fmt.Printf("Warning: Failed to mark workflow-name flag as required: %v\n", err)
	}
	if err := schedulerExecuteWorkflowCmd.MarkFlagRequired("execution-id"); err != nil {
		fmt.Printf("Warning: Failed to mark execution-id flag as required: %v\n", err)
	}
...
func runSchedulerExecuteWorkflow(cmd *cobra.Command, args []string) error {
	...
	if mcpProxyAPIKey == "" && cfg.Servers != nil {
		// Check if global proxy auth has API key configured
		if cfg.ProxyAuth.Enabled && cfg.ProxyAuth.APIKey != "" {
			mcpProxyAPIKey = cfg.ProxyAuth.APIKey
		}
	}
	if mcpProxyAPIKey == "" {
		return fmt.Errorf("MCP proxy API key not found - set MCP_PROXY_API_KEY environment variable or configure in matey.yaml")
	}
