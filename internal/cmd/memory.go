...
func enableMemoryServer(configFile string, cfg *config.ComposeConfig) error {
	...
	if cfg.Memory.DatabaseURL == "" {
		dsn, err := service.BuildDefaultPostgresDSN("memory_graph")
		if err != nil {
			fmt.Fprintln(os.Stderr, "ERROR: cannot enable memory server without POSTGRES_PASSWORD.")
			fmt.Fprintln(os.Stderr, "       Set POSTGRES_PASSWORD directly, or wire a Kubernetes Secret via")
			fmt.Fprintln(os.Stderr, "       valueFrom.secretKeyRef so POSTGRES_PASSWORD is populated in the env.")

			return fmt.Errorf("memory server: %w", err)
		}
		cfg.Memory.DatabaseURL = dsn
	}
	...
	if cfg.Memory.PostgresPassword == "" {
		// Never default to a hardcoded plaintext password: require operators
		// to supply POSTGRES_PASSWORD explicitly (typically via a Kubernetes
		// Secret mounted as an env var).
		pw := os.Getenv("POSTGRES_PASSWORD")
		if pw == "" {
			fmt.Fprintln(os.Stderr, "ERROR: POSTGRES_PASSWORD is unset; refusing to use a plaintext default.")
			fmt.Fprintln(os.Stderr, "       Set POSTGRES_PASSWORD directly, or wire a Kubernetes Secret via")
			fmt.Fprintln(os.Stderr, "       valueFrom.secretKeyRef so POSTGRES_PASSWORD is populated in the env.")

			return fmt.Errorf("memory server: %w", service.ErrMissingPostgresPassword)
		}
		cfg.Memory.PostgresPassword = pw
	}
	if cfg.Memory.CPUs == "" {
		cfg.Memory.CPUs = "1.0"
	}
