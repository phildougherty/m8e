...
// MateyNamespace is the single source of truth for the Kubernetes namespace
// matey installs into and operates on. Every command flag default and every
// `namespace == ""` fallback must use this value. A prior split — `install`
// defaulting to "matey" while the k8s managers fell back to "default" — meant
// `matey install && matey up` silently created resources in one namespace and
// looked for them in another, with no error.
const MateyNamespace = "matey"
