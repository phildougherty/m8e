...
// MCPVersion represents the current Model Context Protocol version (the one
// this server advertises in initialize responses).
//
// The wire protocol also accepts older revisions during initialize so that
// clients that have not yet upgraded can still negotiate a session. See
// AcceptedMCPVersions and the initialize handler in standard_methods.go.
//
// Accepted on the wire: "2024-11-05", "2025-03-26", "2025-06-18".
const MCPVersion = "2025-06-18"

// AcceptedMCPVersions lists every protocol revision this server will accept
// from a client during initialize. When a client advertises one of these the
// server will respond with MCPVersion (the version it actually speaks). When
// a client advertises something else the server still responds with
// MCPVersion and logs a warning; the client decides whether to continue.
var AcceptedMCPVersions = []string{
	"2024-11-05",
	"2025-03-26",
	"2025-06-18",
}

// IsAcceptedMCPVersion reports whether the given protocol version string is
// one this server knows how to speak.
func IsAcceptedMCPVersion(version string) bool {
	for _, v := range AcceptedMCPVersions {
		if v == version {
...
//
// Fields added for 2025-06-18:
//   - Elicitation: client capability. Presence (non-nil) signals the client
//     supports server-initiated elicitation/create requests.
//   - Completions: server capability. Presence signals the server can answer
//     completion/complete requests for prompt arguments / resource URIs.
//
// Both new capabilities follow the spec's "marker object" pattern -- a struct
// with no fields whose mere presence indicates support.
type CapabilitiesOpts struct {
	...
	Elicitation *ElicitationOpts `json:"elicitation,omitempty"`
	Completions *CompletionsOpts `json:"completions,omitempty"`
}

// ElicitationOpts is the marker capability advertised by clients that can
// handle server-initiated elicitation/create requests. Presence = supported;
// no fields defined in the spec yet.
type ElicitationOpts struct{}

// CompletionsOpts is the marker capability advertised by servers that can
// answer completion/complete requests. Presence = supported.
type CompletionsOpts struct{}
...
const (
	...
	// MethodElicitationCreate is the server -> client method for asking the
	// human user a structured question mid-request (introduced 2025-06-18).
	...
	// NotificationMessage is the server -> client log notification
	// ("notifications/message") used together with logging/setLevel.
