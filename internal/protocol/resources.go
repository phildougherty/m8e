...
// generateContentHash generates a hash for content. SHA-256 is used for
// content-integrity / cache-key purposes only (not authentication).
func (rm *ResourceManager) generateContentHash(content string) string {
	return hashHex([]byte(content))
}

// hashHex returns the lowercase-hex SHA-256 of b. The hash is used as a
// content-integrity / cache key (not authentication), so SHA-256 is the
// appropriate primitive; this replaces the previous MD5 implementation.
