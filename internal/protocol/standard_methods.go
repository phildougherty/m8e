...
//
// Protocol version negotiation follows the MCP spec: the server accepts any
// version it knows how to speak (see AcceptedMCPVersions). The response
// always advertises the version this server has chosen (MCPVersion). If the
// client speaks an unknown version we still return our chosen version and
// log a warning -- it is the client's choice whether to proceed.
func (h *StandardMethodHandler) handleInitialize(params json.RawMessage, requestID interface{}) (*MCPResponse, error) {
	...
	// Soft protocol version negotiation. Per the 2025-06-18 spec, an empty
	// or unknown version is not a fatal error -- the server simply tells the
	// client which version it actually supports and the client decides.
	if initParams.ProtocolVersion == "" {
		if h.logger != nil {
			h.logger.Warning("initialize: client did not send protocolVersion; responding with %s", MCPVersion)
		}
	} else if !IsAcceptedMCPVersion(initParams.ProtocolVersion) {
		if h.logger != nil {
			h.logger.Warning("initialize: client requested unsupported protocolVersion %q; responding with %s",
				initParams.ProtocolVersion, MCPVersion)
		}
	}
...
//
// Per the 2025-06-18 negotiation rules, ProtocolVersion must be present but
// is NOT required to match MCPVersion exactly. Any version this server
// knows about (see AcceptedMCPVersions) is accepted; unknown versions are
// allowed too and produce a soft negotiation in handleInitialize (this
// function intentionally does not fail in that case, leaving the dispatcher
// to log a warning and respond with our chosen version).
func ValidateInitializeRequest(params InitializeParams) error {
	...
	// We deliberately do NOT reject mismatched versions here -- the server
	// performs soft version negotiation in handleInitialize.
	_ = IsAcceptedMCPVersion(params.ProtocolVersion)
