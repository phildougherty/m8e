...
func (wst *WebSocketTransport) Start() error {
	// Connect to WebSocket. The HTTP response is closed eagerly: on success
	// gorilla/websocket has already hijacked the connection (body is a no-op
	// closer); on failure we still want to release any pending body bytes.
	conn, resp, err := websocket.DefaultDialer.Dial(wst.url, nil)
	if resp != nil {
		_ = resp.Body.Close()
	}
	if err != nil {
