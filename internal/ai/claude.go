...
func (p *ClaudeProvider) StreamChat(ctx context.Context, messages []Message, options StreamOptions) (<-chan StreamResponse, error) {
	...
	go func() {
		...
		// Close the body unconditionally on goroutine exit. Errcheck-clean
		// pattern: ignore the close error since we are already returning
		// (and an unread body was previously the real leak risk).
		...
		if resp.StatusCode != http.StatusOK {
			...
			if readErr != nil {
				responseChan <- StreamResponse{
					Error: NewProviderError(p.Name(), fmt.Sprintf("HTTP %d (failed to read error body: %v)", resp.StatusCode, readErr), "http_error"),
				}
				return
			}
			responseChan <- StreamResponse{
