...
func (c *K8sComposer) Restart(serviceNames []string) error {
	...
	for _, serviceName := range serviceNames {
		...
		default:
			...
			// Wait for the service's deployment to actually be removed before
			// recreating it, instead of guessing with a fixed sleep.
			c.waitForDeploymentDeleted(serviceName, 10*time.Second)
...
func (c *K8sComposer) restartAllServices() error {
	...
	// Wait for matey-managed deployments to actually be removed before
	// recreating them, instead of guessing with a fixed sleep.
	c.logger.Info("Waiting for cleanup...")
	c.waitForMateyDeploymentsDeleted(15 * time.Second)
...
// waitForDeploymentDeleted polls until the named Deployment is gone from the
// namespace or the timeout elapses. It is best-effort: a timeout is logged but
// not treated as fatal, since callers proceed to recreate the resource anyway.
func (c *K8sComposer) waitForDeploymentDeleted(name string, timeout time.Duration) {
	ctx := context.Background()
	err := wait.PollUntilContextTimeout(ctx, 200*time.Millisecond, timeout, true, func(ctx context.Context) (bool, error) {
		deployment := &appsv1.Deployment{}
		getErr := c.k8sClient.Get(ctx, client.ObjectKey{Name: name, Namespace: c.namespace}, deployment)
	...
	if err != nil {
		c.logger.Warning("Timed out waiting for deployment %s to be deleted: %v", name, err)
	}
}

// waitForMateyDeploymentsDeleted polls until no matey-managed Deployments remain
// in the namespace or the timeout elapses. Best-effort, like waitForDeploymentDeleted.
func (c *K8sComposer) waitForMateyDeploymentsDeleted(timeout time.Duration) {
	ctx := context.Background()
	err := wait.PollUntilContextTimeout(ctx, 200*time.Millisecond, timeout, true, func(ctx context.Context) (bool, error) {
		deployments := &appsv1.DeploymentList{}
		listErr := c.k8sClient.List(ctx, deployments, client.InNamespace(c.namespace), client.MatchingLabels{"app": "matey"})
		if listErr != nil {
			return false, nil
		}
	...
	if err != nil {
		c.logger.Warning("Timed out waiting for matey deployments to be deleted: %v", err)
	}
}
...
func (c *K8sComposer) Status() (*ComposeStatus, error) {
	...
	// Probe the API server once up front so we can surface a single clear
	// error if the kubeconfig is broken — typically a stale CA from a
	// previous k3s install. Without this probe the per-service loop below
	// fires N independent k8s calls and logs the same TLS failure N times,
	// which buries the actionable hint under spam.
	if c.k8sClient != nil {
		probeCtx, probeCancel := context.WithTimeout(context.Background(), 5*time.Second)
		var ns corev1.Namespace
		err := c.k8sClient.Get(probeCtx, client.ObjectKey{Name: c.namespace}, &ns)
		probeCancel()
		if err != nil && kube.IsCertVerifyError(err) {
			return nil, kube.WrapAPIError(err)
		}
		// Other probe errors are not fatal — the namespace may not exist
		// yet, or List permissions may differ from Get. Let the loop below
		// surface those per-service.
	}
	...
	// Check memory service status. memoryManager.Status() returns an error
	// when no MCPMemory resource exists yet — that's not really an "error",
	// it just means the operator hasn't run `matey up memory`. Map it to
	// "stopped" so the `ps` table reads honestly (matching how an MCPServer
	// without a Deployment classifies). Other errors (API unreachable,
	// permission denied) still surface as "error".
...
func (c *K8sComposer) getEnhancedServiceInfo(serviceName, serviceType string) *EnhancedServiceInfo {
	...
	if serviceType == "matey-core" {
		...
		// For core services, use pod readiness as health status. Previously
		// the proxy specifically got an extra HTTP probe against
		// http://localhost:9876 + cluster-internal DNS (matey-proxy /
		// matey-proxy.<ns>.svc.cluster.local). That probe runs from
		// wherever the CLI is invoked — usually an operator's laptop where
		// none of those URLs resolve — so a healthy proxy was being falsely
		// downgraded from "healthy" to "degraded" on every `matey ps` from
		// outside the cluster. The kubelet readinessProbe on the proxy
		// Deployment already does the equivalent HTTP check from inside,
		// so trust the pod state we already computed.
...
func (c *K8sComposer) checkServiceViaProxy(serviceName string) (connected bool, health string) {
	...
	defer func() { _ = resp.Body.Close() }() // best-effort close; we only inspect status
...
func (c *K8sComposer) startMCPServer(name string, serverConfig config.ServerConfig) error {
	...
	// Retry logic for resource conflicts using exponential backoff instead of a
	// hand-rolled sleep loop. The condition func returns (done, err): a retriable
	// conflict yields (false, nil) to trigger another backoff step.
	backoff := wait.Backoff{Steps: 3, Duration: time.Second, Factor: 2.0}
	var lastErr error
	err := wait.ExponentialBackoff(backoff, func() (bool, error) {
		// Check if MCPServer already exists
		existingServer := &crd.MCPServer{}
		getErr := c.k8sClient.Get(ctx, client.ObjectKey{
			Name:      name,
			Namespace: c.namespace,
		}, existingServer)
		...
		if getErr != nil {
			if client.IgnoreNotFound(getErr) == nil {
				...
				if createErr := c.k8sClient.Create(ctx, mcpServer); createErr != nil {
					lastErr = fmt.Errorf("failed to create MCPServer %s: %w", name, createErr)
			...
			lastErr = fmt.Errorf("failed to check if MCPServer %s exists: %w", name, getErr)
			return false, nil // Retry
		}
		...
		if updateErr := c.k8sClient.Update(ctx, mcpServer); updateErr != nil {
			lastErr = fmt.Errorf("failed to update MCPServer %s: %w", name, updateErr)
...
func (c *K8sComposer) stopMCPServer(name string) error {
	...
	// Give the controller a chance to clean up by polling until the MCPServer
	// is actually gone (or a short timeout elapses), then fall through to manual
	// cleanup. This replaces a fixed sleep with a condition-driven wait.
	_ = wait.PollUntilContextTimeout(ctx, 100*time.Millisecond, 1*time.Second, true, func(ctx context.Context) (bool, error) {
		check := &crd.MCPServer{}
		getErr := c.k8sClient.Get(ctx, client.ObjectKey{Name: name, Namespace: c.namespace}, check)
		if getErr != nil && client.IgnoreNotFound(getErr) == nil {
			return true, nil // gone
		}
		return false, nil
...
// GatherStatus collects the status of all services. It is the data-gathering
// half of List, split out so callers like `ps --watch` can poll repeatedly
// without re-implementing the rendering logic.
func GatherStatus(configFile string) (*ComposeStatus, error) {
	return Status(configFile)
}

// RenderStatus writes a human-readable status table to w. It is pure: given a
// ComposeStatus it produces deterministic output, which keeps it easy to test
// and lets the watch loop re-render in place on each tick.
func RenderStatus(w io.Writer, status *ComposeStatus) {
	...
	// Write errors here are not actionable: w is os.Stdout in production
	// (SIGPIPE handles broken pipes) or a *bytes.Buffer in tests (never errors).
	_, _ = fmt.Fprintf(w, "Services (%d total, %d running)\n", serviceCount, runningCount)
	_, _ = fmt.Fprintln(w, strings.Repeat("=", 130))

	// Print header
	_, _ = fmt.Fprintf(w, "%-25s %-10s %-15s %-19s %-12s %-12s %-8s\n",
	...
	for _, name := range serviceNames {
		...
		_, _ = fmt.Fprintf(w, "%-25s %-10s %-15s %-19s %-12s %-12s %-8d\n",
			name,
			svc.Status,
			svc.Type,
			startTime,
...
func List(configFile string) error {
	status, err := GatherStatus(configFile)
	if err != nil {
		return err
	}
