...
func (fd *FileDiscovery) gitignorePatternToRegex(pattern string) (*regexp.Regexp, error) {
	...
	// Detect a trailing-slash directory pattern up front, but do NOT append the
	// regex suffix yet: doing so before the wildcard substitutions below would
	// corrupt the "match everything under the dir" portion (the `.` in `.*`
	// would be escaped and the `*` rewritten to `[^/]*`).
	...
	// Now that wildcard substitution is done, append the directory suffix so
	// the directory itself and anything nested under it are matched.
	if dirPattern {
		pattern = pattern + "($|/.*)"
	}
...
func (fd *FileDiscovery) GetRecentCommits(limit int) ([]map[string]interface{}, error) {
	...
	defer func() {
		commitIter.Close() // Close doesn't return an error for this iterator type
	}()
