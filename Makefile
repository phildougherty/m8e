...
.PHONY: build clean test test-coverage test-race lint fmt vet security-scan docker-build help install uninstall dev deps clean-all build-voice install-voice quickstart helm-lint helm-template helm-test
...
# Quick install: orchestrates a cluster (k3s if none) and deploys m8e via Helm.
quickstart:
	@./scripts/install.sh

# Lint the Helm chart. Catches templating issues without needing a cluster.
helm-lint:
	@echo "Linting Helm chart..."
	helm lint charts/matey/

# Render the Helm chart to stdout for inspection.
helm-template:
	@echo "Rendering Helm chart..."
	helm template matey charts/matey/ -n matey --debug

# Render the Helm chart and dry-run apply against the current cluster context.
# Requires kubectl to be configured against a reachable cluster.
helm-test:
	@echo "Rendering and dry-run applying Helm chart against current cluster..."
	helm template matey charts/matey/ -n matey | kubectl apply --dry-run=client -f -
...
help:
	...
	@echo ""
	@echo "Quick start:"
	@echo "  quickstart      - Install m8e (auto: k3s if no cluster, else current context)"
	@echo ""
	@echo "Build targets:"
	...
	@echo ""
	@echo "Helm targets:"
	@echo "  helm-lint       - Lint the Helm chart in charts/matey/"
	@echo "  helm-template   - Render the Helm chart to stdout for inspection"
	@echo "  helm-test       - Dry-run apply rendered chart against current cluster"
	@echo ""
