...
> **Maturity note (v0.0.4):** `ENABLE_WORKFLOW_ENGINE` enables an experimental, mid-migration feature; `ENABLE_AUDIT_LOGGING` enables in-memory-only audit logging (not persisted); `ENABLE_MEMORY_SERVICE` is partial. Matey has no web UI — `ENABLE_DASHBOARD` does not enable one. See the [Feature Status table in the README](../../README.md#feature-status).

```bash
# Core Features
ENABLE_AI_INTEGRATION=true
ENABLE_WORKFLOW_ENGINE=true    # experimental
ENABLE_MEMORY_SERVICE=true     # partial
ENABLE_TASK_SCHEDULER=true
ENABLE_AUDIT_LOGGING=true      # in-memory only, not persisted
ENABLE_METRICS=true
ENABLE_TRACING=true
