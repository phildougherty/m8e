...
> **Maturity note:** Not all feature flags below back fully implemented features. As of v0.0.4:
> - `workflow_engine` — ⚠️ experimental, mid-migration; works only via the k8s client against a deployed MCPTaskScheduler.
> - `audit_logging` — ⚠️ in-memory only; file/database backends are not implemented and audit history is lost on restart.
> - `memory_service` — ⚠️ partial; CRUD and full-text search work, no versioned migrations.
> - `dashboard` — Matey has no web UI; this flag does not enable one.
> See the [Feature Status table in the README](../../README.md#feature-status).

```yaml
features:
  ai_integration: true
  workflow_engine: true   # experimental
  memory_service: true    # partial
  task_scheduler: true
  audit_logging: true     # in-memory only
  metrics: true
  tracing: true
...
- [ai-workflow.yaml](examples/ai-workflow.yaml) - AI-powered workflows (workflow engine is experimental)
